describe(`TC-FE-PB26-ADD-SALE-ITEM-BY-SELLER-1\n 
    ...
    it(`The sale item table should be empty and the page show "no sale item".`,()=>{
    ...
    it(`[step 2] should have "Add Sale Item" button and click to open the page for entry new sale item.\n
        ...
        cy.get('@modelInput').type('Galaxy Z Fold7') ;

        cy.get('.itbms-price').as('priceInput') ;
        cy.get('@priceInput').type('64900') ;

        cy.get('.itbms-description').as('descriptionInput') ;
        cy.get('@descriptionInput').type('Samsung Galaxy Z Fold 7 จอใหม่ใหญ่ขึ้น แต่เบาบางลงมากกว่า มาพร้อมกล้อง 200MP ') ;
    ...
    it(`[step 5] should show the sale-items list page with 1 items.\n
        should have the item 'Samsung, Galaxy Z Fold7'.`,()=>{
        cy.get('.itbms-row').should('have.length',1)

        cy.contains('.itbms-model','Galaxy Z Fold7').should('exist').as('newItem') ;
        cy.get('@newItem').parent().contains('.itbms-brand','Samsung')
        cy.get('@newItem').parent().contains('.itbms-ramGb','12')
        cy.get('@newItem').parent().contains('.itbms-storageGb','256')
        cy.get('@newItem').parent().contains('.itbms-color','Blue Shadow')
        cy.get('@newItem').parent().contains('.itbms-price','64,900') ;
    })
